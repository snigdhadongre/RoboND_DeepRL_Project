	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		if (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM ) == 0)
                  ...
                  const bool collisionCheck = (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0);
...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	float velocity = vel[action/2] + (1 - 2 * (action % 2)) * actionVelDelta; // TODO - Set joint velocity based on whether action is even or odd.
